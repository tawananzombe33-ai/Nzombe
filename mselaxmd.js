...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const systemMessage = { role: 'system', content: 'You are called mr b² bot. Developed by mr b² bot. You respond to user commands. Only mention developer name if someone asks.' };
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*Mr b² smile 😁-BOT WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ Mr b²-𝙱𝙾𝚃 is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ Mr b²-𝙱𝙾𝚃 Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("Mr b²-𝙱𝙾𝚃 is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading Mr b²-𝙱𝙾𝚃 Commands ...\n");
                fs.readdirSync(__dirname + "/mselaxmd").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            require(__dirname + "/mselaxmd/" + fichier);
